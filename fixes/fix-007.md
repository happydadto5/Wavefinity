...
- A connector position must be a whole multiple of WAVE_LENGTH.
- make_side_connector() returns geometry centered locally; its position argument controls wave/lock phase rather than translating the returned mesh along the wall.
...
- CORNER_CONNECTOR_LEG_LENGTH = LOCKED_CONNECTOR_LENGTH / 2.0
  - current value: 6.0 mm
- CORNER_CONNECTOR_LEG_CENTER = WAVE_LENGTH
  - current value: 4.0 mm

Do not create new tolerance, wall-width, arm-depth, or lock constants for corner connectors.

Why these exact numbers matter:
- one corner leg is a 6 mm section of the existing side connector;
- make_side_connector requires the phase position to be a whole 4 mm wave;
- a 6 mm leg centered at +4 mm occupies +1 through +7 mm after it is translated;
- centered at -4 mm it occupies -7 through -1 mm;
- therefore even on the minimum 8 mm Wavefinity grid span, the leg stays exactly 1 mm away from both corners, matching CORNER_INSET;
- because all Wavefinity bin dimensions are multiples of 8 mm and WAVE_LENGTH is 4 mm, this same part stays on the global wave/lock lattice regardless of which legal bin size surrounds the junction.

### 1.3 Add a private one-leg composer

Add a small private helper conceptually equivalent to:
...
- use a canonical geometry box whose x and y are at least MIN_JOINABLE_SIZE while preserving the caller's wall/base/height/other physical values;
- the cleanest implementation is dataclasses.replace(box, x=max(box.x, MIN_JOINABLE_SIZE), y=max(box.y, MIN_JOINABLE_SIZE)); import replace from dataclasses if needed;
- phase_position = direction * CORNER_CONNECTOR_LEG_CENTER;
- call make_side_connector(canonical_box, connector, axis, position=phase_position, length=CORNER_CONNECTOR_LEG_LENGTH, auto_adjust=False);
- do not pass different bin heights;
- translate the returned mesh along its axis by the same phase_position:
  - axis "x": translate (phase_position, 0, 0)
  - axis "y": translate (0, phase_position, 0)
- return the translated mesh.

The explicit translation is required because make_side_connector's position selects wave phase but the returned connector body remains centered at local zero.

Do not relax make_side_connector's whole-wave position validation.

### 1.4 Add make_corner_connector(box, connector, ways)
...
- let the reused make_side_connector legs retain all existing shallow-bin/base/flat-band validation;
- compute inner_hw, outer_hw = connector_half_widths(box, connector) only for sizing the central cap hub;
...
That canonical shape joins NW + NE through the north seam and NW + SW through the west seam, thereby joining exactly three occupied quadrants. The user rotates the printed part for a different empty quadrant.
...
This composition is intentional: it reuses the exact existing wall channel, lock notches, arm depth, tolerance, and wall-thickness adaptation instead of inventing a second mating profile.

### 1.5 Do not alter existing side behavior

No intended geometry changes to make_side_connector(), connector_half_widths(), lock geometry, different-height web logic, or ordinary side filenames are part of this fix.

If a refactor is absolutely required to expose an existing private helper, the pre-existing side connector output for the same inputs must remain behaviorally unchanged.
...
6. Minimum 8 x 8 mm bins:
   corner connector is still legal because each leg occupies only 1..7 mm or -7..-1 mm from the junction; do not apply the ordinary "minimum 16 mm full side connector" limitation to the corner junction.
...
- 3-Way Corner connector exists and physically composes two one-sided, lock-bearing side-connector legs plus a cap-only center hub.
- 4-Way Corner connector exists and physically composes four one-sided, lock-bearing side-connector legs plus a cap-only center hub.
