...
- the per-leg helper obtains connector_half_widths(box, connector); do not create another wall-fit width calculation here.
...
### 1.7 Add seated corner-fit validation

Corner geometry needs the same fail-before-export protection that generate_side_file() gets from validate_side_fit(). Do not rely only on "watertight" mesh checks.

Add a small helper for the canonical installed bins around a junction, conceptually:

installed_corner_boxes(box, ways)

Use the physical box mesh from make_box(box), translated to these centers around junction (0, 0):
- NW: (-box.x / 2, +box.y / 2)
- NE: (+box.x / 2, +box.y / 2)
- SW: (-box.x / 2, -box.y / 2)
- SE: (+box.x / 2, -box.y / 2)

For ways == 3, return NW + NE + SW, matching the canonical missing-SE connector.
For ways == 4, return all four.
Reject any other ways value.
...
Behavior:
- seat the unflipped connector with translation (0, 0, box.z - connector.arm_depth), which places the underside of its cap at the equal-height bin rims;
- sum intersection_volume(seated, bin) across the installed corner boxes;
- if overlap > 0.01 mm^3, raise RuntimeError with a clear corner-connector collision message;
- otherwise return the overlap.

This check is important because a compact leg begins immediately after the existing corner inset and its arm can approach the perpendicular wall/corner chord. If that physical geometry does not clear the real bin, do not suppress the check. Correct the compact corner geometry while preserving:
- the same wall-following channel profile;
- equal-height behavior;
- no side lock notches;
- the 0.5 mm opposite-corner midpoint gap on an 8 mm wall.

Do not alter global bin corner geometry to make the connector pass.

### 1.8 Do not alter existing side behavior
...
The explicitly required mechanical extractions above (corridor polygon + arm/base clearance validation) are allowed only so corner geometry reuses existing physical rules. The pre-existing side connector output for the same inputs must remain behaviorally unchanged.
...
- **do not inherit hidden Side/Different settings**. Build the corner part with canonical ConnectorSpec defaults: LOCKED_TOLERANCE, LOCKED_CONNECTOR_HEIGHT, DEFAULT_CAP_THICKNESS, and DEFAULT_ARM_THICKNESS. In particular, a stale custom side arm thickness must never widen a corner connector;
...
- leave any previously typed side-only tolerance/length/arm values available for when the user switches back to Side, but understand that the backend deliberately ignores those hidden values for corner geometry and uses the canonical ConnectorSpec defaults.
...
7. capture the current Space/folder identity before starting an asynchronous inventory read. Immediately before showing the toast, confirm the user is still in that same Space/folder and that state.design.box.wall still matches the wall value that was checked. If context changed, discard the stale warning;
...
- Opposite/adjacent corner connectors do not overlap on the minimum 8 mm grid, including at the current maximum 2.4 mm wall; the specified 0.5 mm longitudinal midpoint gap is preserved.
...
- Corner connectors use canonical connector tolerance/height/cap/arm dimensions and cannot inherit stale hidden Side/Different arm/tolerance/length settings.
