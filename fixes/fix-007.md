...
- build a top-only square hub:
  - 2D square corners at (-outer_hw,-outer_hw), (+outer_hw,-outer_hw), (+outer_hw,+outer_hw), (-outer_hw,+outer_hw);
  - extrude by connector.cap_thickness;
  - translate its bottom to z = connector.arm_depth;
  - this hub is cap only: it must not add hanging arms inside the physical corner void.
...
That canonical shape joins NW + NE through the north wall channel and NW + SW through the west wall channel, thereby joining the three occupied quadrants. The user rotates the printed part for a different empty quadrant.
...
Union the legs and hub and return the normal cleaned/manifold result using the same cleanup helper already used by connector geometry. Do not merely concatenate overlapping solids.
...
- 3-Way Corner connector exists and physically composes two compact perpendicular, wall-adaptive friction-grip legs plus a cap-only center hub.
- 4-Way Corner connector exists and physically composes four compact wall-adaptive friction-grip legs plus a cap-only center hub.
