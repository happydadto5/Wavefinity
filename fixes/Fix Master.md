...
- **Status:** Needs remediation work — Correction 7
- **Fix file:** `/fixes/fix-004.md`
- **Implementation thread:** `Space Onboarding Redesign Imp #4`
- **Notes:** Claude commit `f469495b` fixed the Correction 6 targets and preserved the Render/module-route repair, but outside review found remaining Fix 004 gaps. Hosted Create/Configure still derives the inventory filename from the previously-active folder and does not activate the selected browser folder; Create can still treat an existing typed Space with `needs_setup=true` as a new target; existing untyped inventory selected during Create lacks the required explicit configure-vs-choose-another confirmation; hosted “I don’t know yet” does not persist v4/setup-version Design metadata; hosted Edit updates metadata without updating inventory `layout.space`; stale `SP.configureData` can survive Use Untyped/cancel flows; the cross-type dialog is still marked `hidden`; current backend acceptance still permits writing legacy `kind="box"`; and Surface recovery setup is not prefilled. Correction 7 is appended with exact repairs.
