...
- **Status:** Needs remediation work — Correction 10
- **Fix file:** `/fixes/fix-004.md`
- **Implementation thread:** `Space Onboarding Redesign Imp #4`
- **Notes:** Claude commit `866b912` correctly implemented Correction 9's collision-origin routing and Recents setup gate. Outside review found three remaining UI-flow requirements: the Open Existing unconfigured-folder prompt still lacks the required “Choose another folder” action; collision “Choose another folder” clears its state before launching a replacement picker while leaving the collision prompt visible, so cancelling can leave a dead/misrouted prompt; and preserved legacy multi-drawer typed Spaces still show an ordinary unlabeled Drawer selector rather than clearly marking the selector/drawers as legacy/previous data. Correction 10 is appended as a narrow final onboarding cleanup.
