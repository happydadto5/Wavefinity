...
**PLANNED. DO NOT IMPLEMENT UNTIL FIX 010 IS MERGED INTO `main`.**

Fix 010 and Fix 011 both touch `web/index.html`, `web/app.js`, and `test_wavefinity_web.py`. Do not implement them concurrently from the same older base. After Fix 010 receives outside review and is merged, sync the resulting current `origin/main`, re-read this file against that code, and then create `fix11`.

The product behavior in this file is decided. If Fix 010 changes nearby code shape, adapt the patch to the merged code without changing this behavior.
...
1. Update this file on `fix11` with implementation commit SHA.
2. List files changed.
3. List exact tests run and their results.
4. Note any deviation from this plan and why.
5. Push the outbrief and report the branch head.
6. Do not merge.
