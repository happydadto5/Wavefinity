...
- `python3 run_tests.py` (whole suite) — started; this sandbox's geometry
  tests (trimesh/manifold3d booleans) run materially slower here than a
  normal dev machine, so the run was still in progress when this outbrief was
  written. Report the result of that run before treating Fix 011 as accepted
  if it has not finished by the time this is reviewed.
